...
function formatDate(isoDate) {
  if (!isoDate) return 'Sin partidas aún'
      ...
      <div class="mt-8">
        ...
        <div v-else class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            ...
            <!-- Stats -->
            <div class="space-y-3">
              <div v-if="isChessStat(stat)" class="flex justify-between items-center bg-slate-50 dark:bg-zinc-950 px-3 py-2 rounded border-l-2 border-l-emerald-400 dark:border-l-emerald-500 border border-slate-100 dark:border-slate-800/50 transition-colors">
                <span class="text-sm text-slate-500 dark:text-slate-400">Puntuajes (Win/Draw/Loss)</span>
                <span class="font-semibold text-emerald-700 dark:text-emerald-300">
                  {{ Number(stat.wins ?? 0) }}/{{ Number(stat.draws ?? 0) }}/{{ Number(stat.losses ?? 0) }}
                </span>
              </div>
